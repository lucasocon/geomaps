class PharmaciesController < ApplicationController
  ...
  def index
    ...
    #@results = Geocoder.search("190.221.204.188").first
    ...
    @hash = Gmaps4rails.build_markers(@pharmacies) do |pharmacy, marker|
      ...
      marker.infowindow "#{pharmacy.description} - #{pharmacy.address}"
      marker.json({:name => pharmacy.name })
    end
  end
  ...
  private
    ...
    def pharmacy_params
      params.require(:pharmacy).permit(:latitude, :longitude, :address, :description, :name)
    end
end
